...
CREATE TABLE recovery_challenge (
  ...
  -- must be nonempty and at most 255 bytes), and whose authority begins
  -- immediately after the scheme with a positive host starts: the first char
  -- after `wss://` must be an ASCII letter/digit (DNS/IPv4 host) or `[` (IPv6
  -- literal). That rejects empty, path/query/fragment, whitespace, colon,
  -- user-info, dot, and hyphen starts by construction. Full canonical
  -- TLS-origin parsing and normalization stay at the M1.3.2 application
  -- boundary; this is only the durable representability/scheme/host floor.
  CONSTRAINT recovery_challenge_canonical_tls_origin_check CHECK (
    ...
    AND substring(canonical_tls_origin from 7 for 1) ~ '^[A-Za-z0-9\[]$'
  ),
