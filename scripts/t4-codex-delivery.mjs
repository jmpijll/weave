...
/**
 * Resolve the physical path of `candidate` component-wise. For each existing
 * component that is a symbolic link, the link is followed — reading and
 * resolving its target against the physical parent even when that target's
 * terminal segment does not yet exist — and the remaining path continues
 * lexically from that physical target. This closes the dangling-symlink gap:
 * a link whose target enters `evidence/` is detected before any directory
 * creation or process spawn, rather than being caught only by a later `mkdir`
 * failing (the OS enforcing the invariant instead of the guard).
 *
 * Never creates or mutates anything; only reads canonical real paths and link
 * targets. A symlink cycle or a resolution chain deeper than 40 links fails
 * closed rather than looping.
 */
export function physicallyResolve(candidate) {
  return physicallyResolvePath(resolve(candidate), 0);
}

/**
 * Resolve `path` (already a canonical absolute path) to its physical form,
 * following symlink components sequentially. When a component is a symlink,
 * its target is resolved against the physical parent — even when that target's
 * terminal segment does not yet exist — and the remaining components are then
 * walked again, so a symlink appearing later in the path is also followed.
 * `depth` bounds symlink-follow recursion so a cycle cannot loop forever.
 */
function physicallyResolvePath(path, depth) {
  if (depth > 40) {
  ...
  let physical = path.startsWith(sep) ? sep : "";
  for (let index = 0; index < components.length; index += 1) {
    const part = components[index];
    const candidate = join(physical, part);
    let stat;
    try {
      ...
      if (error?.code === "ENOENT" || error?.code === "ENOTDIR") {
        let rest = candidate;
        for (let tail = index + 1; tail < components.length; tail += 1) {
          rest = join(rest, components[tail]);
        }
        return rest;
      }
      throw error;
    ...
    if (stat.isSymbolicLink()) {
      ...
      const resolvedTarget = isAbsolute(target) ? target : join(physical, target);
      const remaining = components.slice(index + 1).join(sep);
      const continuation = remaining === "" ? resolvedTarget : join(resolvedTarget, remaining);
      return physicallyResolvePath(resolve(continuation), depth + 1);
    }
    physical = candidate;
  }
  return physical;
